...
class ProcessPoseEvaluationfromCSVFile:     #  Evaluating angle and exercise from CSV file
    
    def __init__(self, dataframe):
        self.df = dataframe
        
        
    def calculate_angle(self, df, point1, point2, point3):
        """
        ...
        for i in range(len(df)):
            x1, y1 = df.iloc[i][point1], df.iloc[i][f'y{point1[1:]}']
            x2, y2 = df.iloc[i][point2], df.iloc[i][f'y{point2[1:]}']
            x3, y3 = df.iloc[i][point3], df.iloc[i][f'y{point3[1:]}']
    ...
    def evaluate_pushup_sequence(self, intervall=10, schwelle=0.03):
        ...
        for i in range(len(df)):
            ...
            try:
                ...
                angle_body = calculate_angle(shoulder, hip, ankle)
            ...
            try:
                ...
                shoulder = [row["x11"], row["y11"]]
                hip = [row["x23"], row["y23"]]
                angle_head = calculate_angle(shoulder, nose, hip)
            ...
            try:
                l_angle = calculate_angle([row["x11"], row["y11"]],
                                          [row["x13"], row["y13"]],
                                          [row["x15"], row["y15"]])
                r_angle = calculate_angle([row["x12"], row["y12"]],
                                          [row["x14"], row["y14"]],
                                          [row["x16"], row["y16"]])
    ...
    def plot_heatmap(self):
        ...
        heatmap_data = self.df[regel_spalten].fillna(False).astype(int).to_numpy().T
